...
import { ShowWithDetails, ShowPerformances } from '../../types';
...
export const ShowManagementModal: React.FC<ShowManagementModalProps> = ({ isOpen, show, onClose, onSaved }) => {
  ...
  const loadCastData = async () => {
    ...
    try {
      ...
      if (allResponse.success) {
        setAvailableCast((allResponse.data || []).map((member: any) => ({
          CastMemberID: member.CastMemberID,
          FullName: member.FullName || `${member.FirstName || ''} ${member.LastName || ''}`.trim(),
          PrimaryEmail: member.PrimaryEmail || '',
        })));
      } else {
        setMessage({ type: 'error', text: allResponse.error || 'Failed to load cast members' });
      }

      if (currentResponse.success) {
        const currentIds = new Set((currentResponse.data || []).map((row: ShowPerformances | any) => row.CastMemberID));
        setSelectedCastIds(currentIds);
      } else {
        setMessage({ type: 'error', text: currentResponse.error || 'Failed to load show cast' });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const castCounts = useMemo(() => {
    return {
      selected: selectedCastIds.size,
      total: availableCast.length,
    };
  }, [selectedCastIds, availableCast.length]);
        ...
        <div className="border-b border-gray-200 px-6 pt-4">
          <div className="flex gap-4">
            ...
            <button
              ...
              className={`pb-3 px-2 font-medium border-b-2 transition-colors ${
                activeTab === 'cast' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}
            >
              Cast ({castCounts.selected})
            </button>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
              <div className="rounded-lg border border-gray-200 p-4">
                <div className="font-medium text-gray-500">Show Date</div>
                <div className="text-gray-900">{new Date(show.ShowDate).toLocaleDateString()}</div>
              </div>
              <div className="rounded-lg border border-gray-200 p-4">
                <div className="font-medium text-gray-500">Show Time</div>
                <div className="text-gray-900">{show.ShowTime}</div>
              </div>
              <div className="rounded-lg border border-gray-200 p-4">
                <div className="font-medium text-gray-500">Status</div>
                <div className="text-gray-900">{show.Status}</div>
              </div>
              <div className="rounded-lg border border-gray-200 p-4">
              ...
              <div className="rounded-lg border border-gray-200 p-4 sm:col-span-2">
                <div className="font-medium text-gray-500">Cast Members</div>
                <div className="mt-2 text-gray-900">
                  {show.CastMembers && show.CastMembers.length > 0
                    ? show.CastMembers.map((member: any, index) => (
                        <span key={index} className="mr-2 inline-block rounded-full bg-blue-100 px-2 py-1 text-xs text-blue-800">
              ...
              <div className="rounded-lg border border-gray-200 p-4 sm:col-span-2">
                <div className="font-medium text-gray-500">Crew Members</div>
                <div className="mt-2 text-gray-900">
                  {show.CrewMembers && show.CrewMembers.length > 0
                    ? show.CrewMembers.map((member: any, index) => (
                        <span key={index} className="mr-2 inline-block rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-700">
                          {member.FirstName} {member.LastName}
                        </span>
                      ))
                    : 'No crew assigned'}
            ...
            <>
              <div className="mb-3 flex items-center justify-between border-b border-gray-200 pb-3">
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    type="checkbox"
                    checked={availableCast.length > 0 && availableCast.every(member => selectedCastIds.has(member.CastMemberID))}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedCastIds(new Set(availableCast.map(member => member.CastMemberID)));
                      } else {
                ...
                {availableCast.map(member => (
                  <label
                    ...
                    className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition-colors ${
                      selectedCastIds.has(member.CastMemberID) ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:bg-gray-50'
                    }`}
                  >
                    <input
                      type="checkbox"
                      checked={selectedCastIds.has(member.CastMemberID)}
                      onChange={() => toggleCast(member.CastMemberID)}
                      className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                    />

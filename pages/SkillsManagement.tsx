...
export const SkillsManagement: React.FC = () => {
  ...
  const handleAddSkill = async () => {
    ...
    if (res.success) {
      ...
      setMessage({ type: 'error', text: res.error || 'Failed to add skill' });
    }
  };
  ...
  const handleUpdateSkill = async (skillId: number) => {
    ...
    if (res.success) {
      ...
      setMessage({ type: 'error', text: res.error || 'Failed to update skill' });
    }
  };
  ...
  if (loading) return <div className="p-6"><div className="text-center"><p className="text-gray-500">Loading skills...</p></div></div>;

  const skillsByLevelAndCategory = skills.reduce((acc, skill) => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-6 bg-white p-6 rounded-xl border shadow-card">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Optional description of what this skill involves" rows={3} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
      ...
      {!showAddForm && !editingSkillId && (
        <div className="mb-6">
          <button onClick={() => setShowAddForm(true)} className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">+ Add New Skill</button>
        </div>
      )}

      <div className="space-y-8">
        {Object.entries(skillsByLevelAndCategory).sort(([levelA], [levelB]) => levelA.localeCompare(levelB)).map(([level, categoriesMap]) => (
          <div key={level} className="bg-white rounded-xl border shadow-card overflow-hidden">
            ...
            <div className="divide-y">
              {Object.entries(categoriesMap).sort(([catA], [catB]) => catA.localeCompare(catB)).map(([categoryName, categorySkills]) => (

  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (activeEnrollId) {
          await loadSkillRatings(activeEnrollId);
        }
      } else {
            ...
            <div className="bg-white rounded-xl shadow-sm border p-5">
              ...
              <div className="space-y-2">
                {[...completedEnrollments, ...droppedEnrollments].map((e, i) => (
                  <div key={(e as any).EnrollmentID ?? i} className="flex items-center justify-between text-sm p-2 rounded-lg hover:bg-gray-50">
                    <div>
              ...
              <div className="space-y-3">
                {notes.map((n, i) => (
                  <div key={n.NoteID ?? i} className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <p className="text-sm text-gray-800">{n.FeedbackText || n.narrative_feedback || 'No note text'}</p>
                    <p className="text-xs text-gray-400 mt-1">{fmtDate(n.NoteDate || n.note_date)}</p>
                  </div>
                ))}
              </div>
            )}
              ...
              {skillsLoading ? (
                ...
                <div className="space-y-5 mt-3">
                  {Object.entries(skillsByCategory).map(([catName, catSkills]) => (
                    <div key={catName}>
                      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{catName}</h3>
                      <div className="space-y-1.5">
                        {catSkills.map((sk: any) => (
                          <div key={sk.CompetencyID ?? sk.SkillID} className="flex items-center justify-between gap-2">
                            <p className="text-sm text-gray-700 min-w-0 truncate">{sk.SkillName || sk.skill_name || 'Unknown Skill'}</p>
                              ...
                              {[1, 2, 3, 4, 5].map(score => (
                                <div
                                  ...
                                  className={`w-6 h-6 rounded text-xs font-semibold border flex items-center justify-center ${
                                    Number(sk.Rating || sk.rating) >= score
